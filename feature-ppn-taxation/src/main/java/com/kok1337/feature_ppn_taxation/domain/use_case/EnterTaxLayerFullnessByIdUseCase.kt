...
internal class EnterTaxLayerFullnessByIdUseCase @Inject constructor(
    private val taxationTaxLayerInMemoryRepository: TaxationTaxLayerInMemoryRepository,
) {
    operator fun invoke(taxLayerId: UUID, fullness: Double?) {
        taxationTaxLayerInMemoryRepository.updateFullness(taxLayerId, fullness)
    }
}